...
class GraphConvLayer(MessagePassing):
    ...
    def tda_norm(self, edge_index, x, target_emb, num_nodes, weight=None):
    ...
    def forward(self, x, edge_index, target_emb=None, weight=None):
        num_nodes = x.size(0)
        
        if self.norm_type == 'tda': 
            edge_index, edge_weight = self.tda_norm(edge_index, x, target_emb, num_nodes, weight=weight)
